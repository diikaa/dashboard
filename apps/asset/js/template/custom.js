...
jQuery(document).ready(function() {
   ...
   // var cssSkin = '../asset/css/style.'+c+'.css';
   // if (jQuery('body').css('direction') == 'rtl') {
   //    cssSkin = '../asset/css/style.'+c+'.css';
   ...
   // Hide alert when close not dismiss this
   jQuery("[data-hide]").on("click", function(){
     jQuery("." + $(this).attr("data-hide")).addClass('hidden');
   });
   ...
   function setActiveNav() {
      var pgurl = window.location.href.substr(window.location.href.lastIndexOf("app_page")),
          tmp_idx = pgurl.indexOf('/',9) > -1 ? pgurl.indexOf('/',9) : pgurl.length,
          pgurl = pgurl.substr(0, tmp_idx);

      $("#main-nav li a").each(function() {
         if ($(this).attr('href').indexOf(pgurl) > -1) {
            $(this).parent().addClass('active');
            if ($(this).parent().parent().attr('class').indexOf('children') > -1) {
